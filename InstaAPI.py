...
class InstagramAPI:
    ...
    DEVICE_SETTINGS = {'manufacturer': 'Xiaomi',
                       ...
                       'android_version': 19,
                       'android_release': '4.4'}
    USER_AGENT = 'Instagram 89.0.0.21.101 Android ({android_version}/{android_release}; 320dpi; 720x1280; {manufacturer}; {model}; armani; qcom; en_US)'.format(**DEVICE_SETTINGS)
    ...
    def sendMessage(self, target_user, session):
    ...
    def sendVideo(self, recipients, filepath):
        uploadId = self.UpId()
        baseheaders  = copy.deepcopy(self.session.headers)
        #Initial Request
        ...
        response = self.session.post(uri, data=videobytes)
        if response.status_code == 200:
            pass
        else:
        ...
        response = self.session.post(confuri, data=content)
        if response.status_code == 200:
            pass
        else:
            tried = 0
    ...
    def approve_pending_thread(self, thread_id):
        data = json.dumps({"_uuid": self.uuid, "_csrftoken": self.token}) # Change
